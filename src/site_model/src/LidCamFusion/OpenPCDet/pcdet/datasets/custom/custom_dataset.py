...
class CustomDataset(DatasetTemplate):
    ...
    def get_infors(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                annotations['name'] = np.array([obj[0] for obj in obj_list])
                annotations['dimensions'] = np.array([[float(obj[10]), float(obj[8]), float(obj[9])] for obj in obj_list]) # lhw(camera) format
                annotations['location'] = np.concatenate([np.array((float(obj[11]), float(obj[12]), float(obj[13])), dtype=np.float32).reshape(1,3) for obj in obj_list], axis=0)
                annotations['rotation_y'] = np.array([float(obj[14]) for obj in obj_list])

                num_objects = len([obj[0] for obj in obj_list])
                ...
                gt_boxes_lidar = np.concatenate([annotations['location'], l, w, h, rots], axis=1 )
