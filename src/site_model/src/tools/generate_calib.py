...
def main(ROOT_DIR: str, config: dict):
    ...
    # Save the file
    calib_dir = os.path.join(ROOT_DIR, config['calib']['calib_dir'])
    os.makedirs(calib_dir, exist_ok=True)
    np.savetxt(os.path.join(calib_dir, 'calib.txt'), calib)
...
def world_to_camera(ROOT_DIR: str, config: dict, pole_name: str, camera_name: str):
    """
...
def camera_to_pixel(ROOT_DIR: str, config: dict, camera_name: str):
    ...
    """
    camera_info_dir = os.path.join(ROOT_DIR, config['calib']['camera_info_dir'])
    K = np.loadtxt(os.path.join(camera_info_dir, 'camera_info.txt'))[4][28:40]
    K = K.reshape(3,4)
