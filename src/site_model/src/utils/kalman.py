...
class KalmanFilter:
    ...
    def __single_update(self, pred_xpt: np.ndarray, pred_cov: np.ndarray, z: np.ndarray, h, H, R):
        K = np.matmul(pred_cov, np.matmul(H.T, np.linalg.inv(np.matmul(H, np.matmul(pred_cov, H.T)) + R)))
        xpt = pred_xpt + np.matmul(K, (z - h(pred_xpt)))
        cov = pred_cov - np.matmul(np.matmul(K, H), pred_cov)
        return xpt, cov

    def __single_comparison(self, pred_xpt: np.ndarray, z: np.ndarray, h):
        pred_z = h(pred_xpt)
        d = np.linalg.norm(z - pred_z)
        print("\033[1;31mCMP (z/x2z):\033[0m", z, pred_z)
        return d

    def __single_match_comparison(self, pred_xpt: np.ndarray, z: np.ndarray):
        d1 = self.__single_comparison(pred_xpt, z[0:3], self.__h_radar)
        d2 = self.__single_comparison(pred_xpt, z[3:5], self.__h_image)
    ...
    def bulk_comparison(self, zms: np.ndarray, zrs: np.ndarray, zis: np.ndarray):
        ...
        cmp_radar = np.array([self.__single_comparison(x, z, self.__h_radar) for x in self.pred_xpt_pile for z in zrs]).reshape(xpt_num, len(zrs))
        cmp_image = np.array([self.__single_comparison(x, z, self.__h_image) for x in self.pred_xpt_pile for z in zis]).reshape(xpt_num, len(zis))
    ...
    def bulk_update(self, exist_idx: np.ndarray, zms: np.ndarray, zrs: np.ndarray, zis: np.ndarray):
        for pred_idx, z_idx in zip(exist_idx[0], exist_idx[1]):
            if z_idx < len(zms):
                ...
                R = 0.1 * np.eye(2)
            xpt, cov = self.__single_update(self.pred_xpt_pile[pred_idx], self.pred_cov_pile[pred_idx], z, H, R)
    ...
    def bulk_new(self, new_idx: np.ndarray, zms: np.ndarray, zrs: np.ndarray, zis: np.ndarray):
        for i in new_idx:
            if i < len(zms):
                # match
                x = np.array([-1.3, -1.8, 0, 0, 0, self.max_id])
            elif i < len(zms) + len(zrs):
                # radar
                zr = zrs[i - len(zms)]
                xw = self.radar_pos[0] - zr[0] * np.sin(zr[1] * np.pi / 180)
                yw = self.radar_pos[1] + zr[1] * np.cos(zr[1] * np.pi / 180)
