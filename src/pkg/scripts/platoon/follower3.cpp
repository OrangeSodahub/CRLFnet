...
nav_msgs::Odometry ldmsg,f4msg,f3msg,f1msg,f2msg,msg1,msg2,msg3,msg4,msg5;
...
void followf1 ()
{
  tf::Quaternion f1orientation,f2orientation,f3orientation;
  ...
  double theta = atan2(f2msg.pose.pose.position.y-f3msg.pose.pose.position.y,
                                 f2msg.pose.pose.position.x-f3msg.pose.pose.position.x) - f2orientation.getAngle();
  double theta1 = atan2(f1msg.pose.pose.position.y-f3msg.pose.pose.position.y,
                                 f1msg.pose.pose.position.x-f3msg.pose.pose.position.x) - f2orientation.getAngle();
 
  double r = sqrt(pow(f3msg.pose.pose.position.x-f2msg.pose.pose.position.x, 2) +
                                  pow(f3msg.pose.pose.position.y-f2msg.pose.pose.position.y, 2));
  double r1 = sqrt(pow(f1msg.pose.pose.position.x-f3msg.pose.pose.position.x, 2) +
                                  pow(f1msg.pose.pose.position.y-f3msg.pose.pose.position.y, 2));
  double alpha = PI/2 - f3orientation.getAngle();
  double e = f3msg.pose.pose.position.x - 1.87;
  double k = 1;
  if(r1 > 0.4 && r > 0.4)
  {
...
void followf2 ()
{
  ...
  double r = sqrt(pow(f3msg.pose.pose.position.x-f2msg.pose.pose.position.x, 2) +
                                  pow(f3msg.pose.pose.position.y-f2msg.pose.pose.position.y, 2));
  double r1 = sqrt(pow(f3msg.pose.pose.position.x-f1msg.pose.pose.position.x, 2) +
                                  pow(f3msg.pose.pose.position.y-f1msg.pose.pose.position.y, 2));
  double alpha = PI/2 - f3orientation.getAngle();
  double e = f3msg.pose.pose.position.x - 1.87;
  double k = 1;
  if(r1 > 0.4 && r > 0.4)
...
void overtake (void)
{
  ...
  double y_ld_f3 = ldmsg.pose.pose.position.y - f3msg.pose.pose.position.y;
  double x_f2_f3 = f2msg.pose.pose.position.x - f3msg.pose.pose.position.x;
  double alpha = f2orientation.getAngle()-f3orientation.getAngle();
  
  if(overtake_phase == 1) 
  {
    overtake_phase = 1;
    double e = 0.25 - x_f2_f3;//left 30cm lane
    double k = 4;

    //f1's speed is a little faster than ld
    vel_msg.drive.speed = getmodule(f2msg.twist.twist.linear) - 0.05;
  ...
  {
    double e = 0.25 - x_f2_f3;//original lane
    double k = 3;
    //f1's speed is a little faster than ld
  ...
  {
    //??
    double e = 0 - x_f2_f3;//original lane
    double k = 3;
    //f1's speed is a little faster than ld
...
void slowdown (void)
{
  ...
  tf::Quaternion f1orientation,f2orientation,f3orientation,f4orientation;
  tf::quaternionMsgToTF(f1msg.pose.pose.orientation, f1orientation);
  tf::quaternionMsgToTF(f2msg.pose.pose.orientation, f2orientation);
  tf::quaternionMsgToTF(f3msg.pose.pose.orientation, f3orientation);
  tf::quaternionMsgToTF(f4msg.pose.pose.orientation, f4orientation);
  //get vertical distance between ld and f3
  double y_f3_f4 = f3msg.pose.pose.position.y - f4msg.pose.pose.position.y;
  double x_f4_f3 = f4msg.pose.pose.position.x - f3msg.pose.pose.position.x;
  double alpha = f2orientation.getAngle()-f3orientation.getAngle();
  
  if(overtake_phase == 1) 
  {
    overtake_phase = 1;
    double e = 0.25 - x_f4_f3;//left 30cm lane
    double k = 2;

    //f1's speed is a little faster than ld
    vel_msg.drive.speed = getmodule(f2msg.twist.twist.linear)+0.04;

    //Stanley method
    vel_msg.drive.steering_angle = atan2(k*e,1) + alpha;
    
    if(abs(alpha) < 0.05 && x_f4_f3 > 0.24 )
    {
      overtake_phase = 2;
    }
  }else if(overtake_phase == 2)
  {
    double e = 0.25 - x_f4_f3;//original lane
    double k = 3;
    //f1's speed is a little faster than ld
    vel_msg.drive.speed = getmodule(ldmsg.twist.twist.linear) - 0.1;
  ...
  {
    //??
    double e = 0 - x_f4_f3;//original lane
    double k = 3;
    ...
    //f1's speed is a little faster than ld
    //vel_msg.drive.speed = getmodule(f4msg.twist.twist.linear);
    if(r > 0.4)
    {
      vel_msg.drive.speed = 0.3 * r;
    }else
