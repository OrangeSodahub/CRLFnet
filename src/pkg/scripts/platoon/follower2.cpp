...
nav_msgs::Odometry ldmsg,f1msg,f2msg,msg1,msg2,msg3,msg4,msg5;
...
void followld ()
{
  tf::Quaternion f1orientation,f2orientation,ldorientation;
  ...
  double theta = atan2(f1msg.pose.pose.position.y-f2msg.pose.pose.position.y,
                                 f1msg.pose.pose.position.x-f2msg.pose.pose.position.x) - f2orientation.getAngle();
  double theta1 = atan2(ldmsg.pose.pose.position.y-f2msg.pose.pose.position.y,
                                 ldmsg.pose.pose.position.x-f2msg.pose.pose.position.x) - f2orientation.getAngle();
 
  double r = sqrt(pow(f1msg.pose.pose.position.x-f2msg.pose.pose.position.x, 2) +
                                  pow(f1msg.pose.pose.position.y-f2msg.pose.pose.position.y, 2));
  double r1 = sqrt(pow(ldmsg.pose.pose.position.x-f2msg.pose.pose.position.x, 2) +
                                  pow(ldmsg.pose.pose.position.y-f2msg.pose.pose.position.y, 2));
  double alpha = PI/2 - f2orientation.getAngle();
  double e = f2msg.pose.pose.position.x - 1.87;
  double k = 0.5;
  if(r1 > 0.4 && r > 0.4)
  {
  ...
  vel_msg.drive.steering_angle = k*theta1;//atan2(k*e,1) + alpha;
...
void followf1 ()
{
  ...
  vel_msg.drive.steering_angle = (k*theta);//atan2(k*e,1) + alpha;
  slave_vel.publish(vel_msg);
}
...
void overtake (void)
{
  ...
  double y_ld_f2 = ldmsg.pose.pose.position.y - f2msg.pose.pose.position.y;
  double x_f1_f2 = f1msg.pose.pose.position.x - f2msg.pose.pose.position.x;
  double alpha = f1orientation.getAngle()-f2orientation.getAngle();
  
  if(overtake_phase == 1) 
  {
    overtake_phase = 1;
    double e = 0.25 - x_f1_f2;//left 30cm lane
    double k = 4;

    //f1's speed is a little faster than ld
    vel_msg.drive.speed = getmodule(f1msg.twist.twist.linear)-0.05;
  ...
  {
    double e = 0.25 - x_f1_f2;//original lane
    double k = 3;
    //f1's speed is a little faster than ld
  ...
  {
    //??
    double e = 0 - x_f1_f2;//original lane
    double k = 3;
    //f1's speed is a little faster than ld
