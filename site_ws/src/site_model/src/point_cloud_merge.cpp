...
int main()
{
	...
	PointCloudT::Ptr cloud_3(new PointCloudT);
	PointCloudT::Ptr cloud(new PointCloudT);
	pcl::io::loadPCDFile("/home/zonlin/ROS/site_ws/src/site_model/point_cloud_data/point_cloud.pcd", *cloud_1);
	pcl::io::loadPCDFile("/home/zonlin/ROS/site_ws/src/site_model/point_cloud_data/bin2pcd/000000.pcd", *cloud_2);

	*cloud = *cloud_1 + *cloud_2;

    if(pcl::io::savePCDFileBinary ("/home/zonlin/ROS/site_ws/src/site_model/point_cloud_data/bin2pcd/integrated.pcd", *cloud)>=0)
    {std::cerr << "Saved point_cloud.pcd" << " " << cloud->points.size() << "points have been merged" << std::endl;}
