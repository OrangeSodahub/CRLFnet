...
int main(int argc, char** argv){
  ...
  slave_vel =
    node.advertise<geometry_msgs::Twist>("/deepracer2/cmd_vel", 10);
  ros::Subscriber laser = node.subscribe("/deepracer2/scan", 1, laserCallBack);
  ...
  while (node.ok()){
    ...
    try{
      listener.waitForTransform("/deepracer2","/deepracer1",ros::Time(0),ros::Duration(3.0));
      listener.lookupTransform("/deepracer2", "/deepracer1",
    ...
    try{
      listener.waitForTransform("/deepracer1","/deepracer2",ros::Time(0),ros::Duration(3.0));
      listener.lookupTransform("/deepracer1", "/deepracer2",
