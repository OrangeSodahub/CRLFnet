...
int main(int argc, char** argv){
  ...
  slave_vel =
    node.advertise<geometry_msgs::Twist>("/deepracer4/cmd_vel", 10);
  ros::Subscriber laser = node.subscribe("/deepracer4/scan", 1, laserCallBack);
  ...
  while (node.ok()){
    ...
    try{
	    listener.waitForTransform("/deepracer4","/deepracer1",ros::Time(0),ros::Duration(3.0));
      listener.lookupTransform("/deepracer4", "/deepracer1",
    ...
    try{listener.waitForTransform("/deepracer1","/deepracer4",ros::Time(0),ros::Duration(3.0));
      listener.lookupTransform("/deepracer1", "/deepracer4",
    ...
    vel_msg.angular.z = 4.0 * atan2(transformSM.getOrigin().y(),
                                   transformSM.getOrigin().x());
    vel_msg.linear.x = 0.167 * sqrt(pow(transformSM.getOrigin().x(), 2) +
