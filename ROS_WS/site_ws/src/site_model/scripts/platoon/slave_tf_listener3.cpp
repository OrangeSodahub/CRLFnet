...
int main(int argc, char** argv){
  ...
  slave_vel =
    node.advertise<geometry_msgs::Twist>("/deepracer3/cmd_vel", 10);
  ros::Subscriber laser = node.subscribe("/deepracer3/scan", 1, laserCallBack);
  ...
  while (node.ok()){
    ...
    try{
	    listener.waitForTransform("/deepracer3","/deepracer1",ros::Time(0),ros::Duration(3.0));
      listener.lookupTransform("/deepracer3", "/deepracer1",
    ...
    try{listener.waitForTransform("/deepracer1","/deepracer3",ros::Time(0),ros::Duration(3.0));
      listener.lookupTransform("/deepracer1", "/deepracer3",
