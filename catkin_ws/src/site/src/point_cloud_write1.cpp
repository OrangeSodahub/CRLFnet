...
//save to PCD for 5 times    
    std::string pcd_name("point_cloud_1"); //velodyne1
    if(i<5)
    {
        i++;
        ch++;
        pcd_name+=ch;
        if(pcl::io::savePCDFileBinary ("/home/zonlin/ROS/catkin_ws/src/site/data/ "+pcd_name+".pcd", cloud)>=0)//input pointcloud should be pcl::PointCloud<PointT> only,rather than others 
...
// Create a ROS subscriber for the input point cloud
  ros::Subscriber sub = nh.subscribe ("/velodyne1_points", 1, call_back);
...
int main (int argc, char **argv)
{
    ros::init (argc, argv, "point_cloud_write1");
    ros::NodeHandle nh;
    ros::Subscriber bat_sub = nh.subscribe("/velodyne1_points", 10, cloudCB); // topic名为pcl_output
    ros::spin();
